...
export function printTableQrCode(opts: {
  ...
  const url = customerOrderUrl(opts.tableId)
  const win = window.open('', '_blank', 'noopener,noreferrer,width=480,height=720')
  if (!win) return

  const safeLabel = opts.tableLabel.replace(/[<>&]/g, '')
  const safeVenue = (opts.venueName || 'EventFlow').replace(/[<>&]/g, '')
...
<body>
  <div class="sheet">
    ...
    <h1>${safeLabel}</h1>
    <div class="cap">Kapacita: ${opts.seatCapacity ?? 4} osob · Objednejte si sami</div>
